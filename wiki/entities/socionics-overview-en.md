---
title: Socionics Overview
type: entity
tags: [socionics, tactical-level, model-a, information-metabolism, overview]
created: 2026-04-15
updated: 2026-08-30
claim_status: [project-definition, source-attribution, research-hypothesis]
claims:
  - id: socionics-tactical-model
    status: research-hypothesis
  - id: socionics-reference-codes
    status: source-attribution
caveat_ids: [typology-is-heuristic, type-is-not-person, no-pair-verdict, context-and-safety-override-type]
sources: [raw/socionics/what-is-socionics.md, raw/socionics/model-a.md, raw/socionics/information-aspects.md]
lang: en
translation_group: socionics-overview
semantic_version: 1
reviewed_semantic_version: 1
document_status: active
page_role: entity
---

# Socionics: Tactical Reference Overview

<!-- section:overview-and-scope -->
## Overview and Scope

<!-- claim:socionics-tactical-model -->
Socionics is a typological tradition associated with Aušra Augustinavičiūtė, Jungian type ideas, information metabolism, and Model A. Before We Build uses it as a **heuristic hypothesis about tactical information modeling and exchange**. It does not cover all communication, the whole personality, or the outcome of a relationship.

<!-- section:core-constructs -->
## Core Constructs

The school distinguishes eight information elements. The questions below are reference prompts, not established modules of the brain or directly observed objects.

| Element | Reference focus |
|---|---|
| Ti | structure, consistency, classification |
| Te | external evidence, effectiveness, procedure |
| Fi | relational distance, commitment, interpersonal evaluation |
| Fe | expressed affect and social atmosphere |
| Si | experienced state, comfort, continuity of sensation |
| Se | force, boundaries, control of resources |
| Ni | temporal pattern, development, trajectory |
| Ne | possibilities, alternatives, potential |

Here “ethics” in Fi/Fe is school terminology for information content; it is not the value-moral foundation and does not measure moral worth.

<!-- section:position-structure -->
## Model A Position Structure

| Position | Common name | Cautious model-internal hypothesis |
|---|---|---|
| 1 | Leading / Base | confident foreground modeling |
| 2 | Creative | flexible production for the situation |
| 3 | Role | conscious situational regulation |
| 4 | Vulnerable / PoLR | sensitive and less flexible regulation |
| 5 | Suggestive | valued external input |
| 6 | Mobilizing | valued development and activation |
| 7 | Ignoring | strong background filtering |
| 8 | Demonstrative | strong background production |

The position names express Model A roles. They do not by themselves establish ability, deficiency, pathology, or a causal mechanism.

<!-- section:type-reference -->
## Type Reference

<!-- claim:socionics-reference-codes -->
The standard reference catalog contains 16 type codes grouped into four quadras:

| Quadra | Type codes |
|---|---|
| Alpha | ILE, LII, ESE, SEI |
| Beta | SLE, EIE, LSI, IEI |
| Gamma | SEE, ILI, LIE, ESI |
| Delta | IEE, EII, SLI, LSE |

The catalog is a formal arrangement of information-element positions. Quadra membership marks model-internal valued elements; it does not prove shared moral values, ease of communication, or relationship quality. Three-letter codes and historical four-letter aliases are reference names, not kinds of people.

<!-- section:observations-and-alternatives -->
## Observations and Alternative Explanations

Direct observations include which information a person requests, supplies, ignores, reformulates, or treats as sufficient in a concrete exchange. Type and function positions are interpretations of repeated indicators. Knowledge, profession, culture, language, status, attachment, stress, neurodivergence, and task demands are rival explanations.

<!-- section:non-inferences -->
## What Must Not Be Inferred

- A type does not determine intelligence, morality, dignity, safety, attachment, vocation, or destiny.
- A “strong,” “weak,” “valued,” or “vulnerable” function is not a clinical or moral judgment.
- An intertype relation does not create a pair score, ranking, recommendation, or verdict.
- A type cannot be established from one behavior, appearance, profession, test result, or public-person impression.

<!-- section:bwb-use -->
## Use in Before We Build

Socionics supplies tactical questions: Which information is missing? Which formulation is being rejected? Are both people using the same word for different priorities? Does a proposed information contact recur across situations? Answers must be checked against observed exchanges and alternative explanations.

<!-- section:sources -->
## Sources and Evidence Status

- `raw/socionics/what-is-socionics.md`
- `raw/socionics/model-a.md`
- `raw/socionics/information-aspects.md`

These sources establish attributed school definitions and codes. They do not by themselves establish psychometric reliability, construct validity, neural realization, or pair outcomes.

<!-- section:next-reading -->
## Next Reading

- [[tactical-compatibility-en]]
- [[socionics-detailed-en]]
- [[socionics-intertype-relations-en]]
- [[compatibility-level-boundaries-en]]
- [[psychosophy-overview-en]]
- [[temporistics-overview-en]]
